...
def normalizar_csv():
    Csv2 = pd.read_csv("PlayStore_Operacion1.csv")
    Comité_Array = np.array(Csv2['ID'])
...
def plt_lr(df: pd.DataFrame, x:str, y: str, m: float, b: float, r2: float, r2_adj: float, low_band: float, hi_band: float, colors: Tuple[str,str]):
    ...
    plt.title("Pronóstico Cantidad de aplicaciones lanzadas el proximo año")
    plt.xticks(rotation=90)

    plt.savefig('Pronostico Aplicaciones', dpi=200)
    plt.show()

df = pd.read_csv("PlayStore_Operacion1.csv")
df2 = df.groupby("Año de lanzamiento")\
              .aggregate(Cantidad=pd.NamedAgg(column="ID", aggfunc=pd.DataFrame.mean))
df2.reset_index(inplace=True)

a = linear_regression(df2, "Año de lanzamiento", "Cantidad")
plt_lr(df=df2, x="Año de lanzamiento", y="Cantidad", colors=('red', 'gray'), **a)
